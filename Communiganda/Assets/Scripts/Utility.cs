...
public class Utility : MonoBehaviour
{
    ...
    private IEnumerator SimpleAnimateCR(SpriteRenderer spriteRend, float intervall, params Sprite[] sprites)
    {
        int index = 0;
        while (true)
        {
    ...
    private IEnumerator SceneTransition(string sceneName, float delayBeforeTrans)
    {
        yield return new WaitForSeconds(delayBeforeTrans);
        SceneManager.LoadScene(sceneName);
    }
    private IEnumerator SceneTransition(int sceneIndex, float delayBeforeTrans)
    {
        yield return new WaitForSeconds(delayBeforeTrans);
        SceneManager.LoadScene(sceneIndex);
    }
    ...
    {
        if (Vector3.Distance(pos1.position, pos2.position) <= tolerance) return true;
        else return false;
    }
